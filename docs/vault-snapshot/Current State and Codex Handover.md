...
This reconciliation records the work and user decisions made through 6 September 2026. Read it before the older project notes. Where a dated reconciliation block identifies a conflict, this note supersedes the historical statement; unaffected design and domain rules remain in effect. Obsidian remains the project documentation source. A future code-folder handover should link here and carry a dated snapshot, rather than silently create a competing source of truth.
...
Prepare a clearly named dedicated code folder containing the working code, relevant audit evidence, saved review export and a small start-here file linking to this vault. Verify the copy before opening it in Codex. Keep source archives, vault assets and the original workspace intact. Do not close the current review environment until the export and handover have been confirmed. Then implement the outstanding teacher feedback and the designed teacher page; no new database migration, paid run or bulk enrichment is authorised merely by this handover.
