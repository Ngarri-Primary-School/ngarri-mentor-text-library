...
A proposed quality guide based on the strongest *Crickwing* entries is awaiting review at `reviews/CONTENT-AUTHORING-GUIDE.ai-draft.md`. It is intentionally not yet part of the canonical specification. Do not revise the remaining pilot-book explanations from it until Phill approves the guide. After approval, incorporate the accepted rules into the canonical content specification and enrichment skill, then revise one remaining pilot book at a time with teacher approval between books. The proposed order is *The Alphabet Tree*, *Night Tree*, *Little Blue and Little Yellow*, then *The Gruffalo*.

The immediate next action is Phill's review of the draft content-authoring guide. Keep all other new material `ai_suggested` until Phill reviews it; do not expose it in teacher-facing pages or filters before approval. No paid generation or bulk enrichment is authorised by the transfer. Steve's invitation and school-account Codex connection remain pending. Supabase and hosting ownership require their own handover.
